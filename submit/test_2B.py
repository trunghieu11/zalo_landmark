...
opt.TEST = edict()
opt.TEST.CHECKPOINT = './model/best_model.pk'
opt.TEST.WORKERS = 8
...
# make submit file
result_file = open("submit.csv", "w")
result_file.write("id,predicted\n")

for fname, label in zip(images, pred_indices):
    result_file.write(fname[:-4] + "," + ' '.join(str(x) for x in label) + "\n")
